...
class Adjustment_Layer(nn.Module):
    ...
    def __init__(
        self, in_channels, input_resolution=None, feature_scale=1, resize_factor=1
    ):
        ...
        if input_resolution is None:
            ...
            if resize_factor > 1:
                self.rescale = nn.ConvTranspose2d(
    ...
    def forward(self, x):
        if self.input_resolution is None:
            ...
            if (
                len(x.shape) < 4
            ):  # This is only needed for ViT. The most recent Swin Transformer implementations maintain inmage dimenionality.
                B, L, C = x.shape
                assert (
                    L == H * W
                ), f"Input feature size ({L}) does not equal expected HxW ({H}*{W})."
                assert (
                    H % 2 == 0 and W % 2 == 0
                ), f"Input feature with size ({H}*{W}) are not equal."
...
class Atrous_Layer(nn.Module):
    ...
    def __init__(
        self, in_channels, out_channels, dilation, kernel_size=3, init_bn=False
    ):
...
class Dense_ASPP(nn.Module):
    ...
    def __init__(
        self, in_channels, out_channels, kernel_size=3, init_kernel=None, init_bn=False
    ):
    ...
    def forward(self, x):
        ...
        concat_aspp = torch.cat(
            [atrous_1, atrous_3, atrous_6, atrous_12, atrous_18, atrous_24], dim=1
        )
...
class Upsample(nn.Module):
    ...
    def forward(self, x):
        return F.interpolate(
            x, scale_factor=self.ratio, mode=self.mode
        )  # Nearest is the fastest upsampling algorithm
        ...
        for c, out_chs in enumerate(out_channels):
            ...
            layers.append(
                nn.Sequential(
                    ...
                    nn.ELU(),
                    Upsample(ratio=ratios[c], mode="nearest"),
                )
            )
