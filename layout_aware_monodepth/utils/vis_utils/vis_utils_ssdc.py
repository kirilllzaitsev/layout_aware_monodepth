...
def display_warping(rgb_tgt, pred_tgt, warped):
    """??? Plot target, predicted adjacent frames and a warping result"""

    def preprocess(rgb_tgt, pred_tgt, warped):
        rgb_tgt = 255 * np.transpose(
            np.squeeze(rgb_tgt.data.cpu().numpy()), (1, 2, 0)
        )  # H, W, C
        ...
        warped = 255 * np.transpose(
            np.squeeze(warped.data.cpu().numpy()), (1, 2, 0)
        )  # H, W, C
        recon_err = np.absolute(warped.astype("float") - rgb_tgt.astype("float")) * (
            warped > 0
        )
